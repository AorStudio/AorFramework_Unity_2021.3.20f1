...
namespace RootMotion.Demos {
	...
	public class MotionAbsorb : OffsetModifier {
		...
		public class Absorber {
			...
			// Set effector position and rotation EffectWeight to match the value, multiply with the EffectWeight of this Absorber
			public void UpdateEffectorWeights(float w) {
				e.positionWeight = w * weight;
				e.rotationWeight = w * weight;
			}

			// Set effector positionOffset to match the position, multiply with the EffectWeight of this Absorber
			public void SetPosition(float w) {
				e.positionOffset += (position - e.bone.position) * w * weight;
			}

			// Set effector bone rotation to match the rotation, multiply with the EffectWeight of this Absorber
			public void SetRotation(float w) {
				e.bone.rotation = Quaternion.Slerp(e.bone.rotation, rotation, w * weight);
			}
		}

		[Tooltip("Use either effector position, position EffectWeight, rotation, rotationWeight or positionOffset and rotating the bone directly.")]
		public Mode mode;
		...
		protected override void OnModifyOffset() {
			...
			// Evaluate the absorb EffectWeight
