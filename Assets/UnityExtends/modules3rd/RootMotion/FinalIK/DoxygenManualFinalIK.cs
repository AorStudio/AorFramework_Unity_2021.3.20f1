...
<b>Getting started:</b>
	...
	- Set EffectWeight to 1, press Play
...
<b>Solver variables:</b>
	...
	- <b>EffectWeight</b> - the solver EffectWeight for smoothly blending out the effect of the IK
	- <b>poleWeight</b> - the EffectWeight of keeping the poleAxis of the AimTransform oriented towards polePosition.
	...
	You can skip bones in the hierarchy and the Aim Transform itself can also be included. The bone hierarchy can not be branched, meaning you cant assing bones from both hands. Bone EffectWeight determines how strongly it is used in bending the hierarchy
...
<b>Bone weights</b>
<BR> Each bone in the "Bones" has a EffectWeight parameter. It determines how much proportionally is a bone used in the solving process. 
Fox example if you do not wish a certain spine bone to bend too much, you can just decrease it's EffectWeight. 
...
<BR> While AimIK EffectWeight is 1, the solver will maintain the weapon oriented at the target at all times. This might not be the desired behaviour while playing a recoil or reloading animation.
...
CCD is very fast and reliable even with rotation limits applied. CCD tends to overemphasise the rotations of the bones closer to the target position (a very long CCD chain would just roll in around it's target). Reducing bone EffectWeight down the hierarchy will compensate for this effect.
...
<b>Getting started:</b>
		...
		- Press Play, set EffectWeight to 1
...
<b>Solver variables:</b>
	...
	- <b>rootPin</b> - the EffectWeight of pinning the first chains to the Transform that the component is attached to
	- <b>chains</b> - the list of FABRIK components used by this FABRIKRoot

<b>Chain variables:</b>
	- <b>IK</b> - the FABRIK component used
	- <b>pull</b> - parent pull EffectWeight. How much does this chain pull it's parent?
...
<b>Pulling, Reaching and Pushing:</b>
		Each chain has the "pull" property. When all chains have pull equal to 1, pull EffectWeight is distributed equally between the limbs. That means reaching all effectors is not quaranteed if they are very far from each other. 
		...
		However, when for instance the left arm chain has pull EffectWeight equal to 1 and all others have 0, you can pull the character from it's left hand to Infinity without losing contact.
...
<b>Limitations:</b>
		...
		- When you move a limb end-effector and the effector rotation EffectWeight is 0, FBBIK will try to maintain the bending direction of the limb as it is animated. When the limb rotates close to 180 degrees from it's animated direction, you will start experiencing rolling of the limb, meaning, the solver has no way to know at this point of singularity, which way to rotate the limb. 
...
<b>Body variables:</b>
	...
	- <b>Position Weight</b> - the position EffectWeight of the body effector. If weighed in, the body will be pinned to solver.bodyEffector.position. This overrides bodyEffector.positionOffset.
	...
	- <b>Pull Body Vertical</b> - EffectWeight of hand effectors pulling the body vertically (relative to root rotation).
	- <b>Pull Body Horizontal</b> - EffectWeight of hand effectors pulling the body horizontally (relative to root rotation).
	...
	- <b>Spine Twist Weight</b> - the EffectWeight of twisting the spine bones gradually to the orientation of the chest triangle. Relatively expensive, so set this to 0 if there is not much spine twisting going on.
...
<b>Limb variables:</b>
	...
	- <b>Position Weight</b> - the position EffectWeight of the effector. If weighed in, the effector bone will be pinned to effector.position. This overrides effector.positionOffset.
	- <b>Rotation Weight</b> - the rotation EffectWeight of the effector. If weighed in, the limb will be rotated to effector.rotation. This also changes the bending direction of the limb. If the bending direction assumed by the solver is disagreeable, set rotation EffectWeight to 0 and either just rotate the hand/foot after FBBIK is done or use a Bend Goal for full precision.
	- <b>Maintain Relative Pos</b> - if 1, the (unweighed) limb will rotate along with the chest/hip triangle.
	- <b>Pull</b> - the EffectWeight of pulling the parent chain. If this limb is the only one to have full pull and the others have none, you will be able to pull the character from that end effector without ever loosing contact.
	- <b>Reach</b> - pulls the first bone of the limb closer to the last bone.
	- <b>Push</b> - the EffectWeight of the end-effector pushing the shoulder/thigh when the end-effector is close to it.
	...
	- <b>Bend Goal Weight</b> - the EffectWeight of bending the limb towards the Bend Goal.
...
<b>Solver variables:</b>
	...
	- <b>positionWeight</b> - the EffectWeight of solving to the target position (IKPosition)
	- <b>rotationWeight</b> - the EffectWeight of solving to the target rotation (IKRotation)
	...
	- <b>maintainRotationWeight</b> - EffectWeight of maintaining the rotation of the third bone as it was before solving
	- <b>bendModifier</b> - a selection of automatic modifiers of the bend normal
	- <b>bendModifierWeight</b> - the EffectWeight of the bend modifier
...
\code
	...
	void LateUpdate () {
		lookAt.solver.IKPositionWeight = 1f; // The master EffectWeight
		...
		// Changing the clamp EffectWeight of individual body parts
...
<b>Solver variables:</b>
	- <b>target</b> - the target Transform. If assigned, solver IKPosition will be automatically set to the position of the target
	- <b>EffectWeight</b> - the master EffectWeight of the solver (multiplies all the other weights)
	- <b>bodyWeight</b> - the EffectWeight of rotating the spine bones
	- <b>headWeight</b> - the EffectWeight of rotating the head bone
	- <b>eyesWeight</b> - the EffectWeight of rotating the eye bones
	...
	- <b>clampSmoothing</b> - the number of sine smoothing iterations applied to clamping to make it smoother
	- <b>spineWeightCurve</b> - normalized EffectWeight distribution between the spine bones. The first spine bone is at "time" 0, the last spine bone is at 1
...
<b>Getting started:</b>
	...
	- Create a PositionWeight EffectWeight curve, that consists of 3 keyframes {(0, 0), (1, 1), (2, 0)}, where x is horizontal and y is vertical value.
...
<b>Component variables:</b>
	...
	- <b>fadeInTime</b> - the time of fading in all the channels used by the interaction (EffectWeight of the effector, reach, pull..)
	...
	<BR><b>weightSpeed</b> - the speed of weighing in/out the LookAtIK EffectWeight.
...
<b>Component variables:</b>
	...
	- <b>weightCurves</b> - The EffectWeight curves define the process of the interaction. The interaction will be as long as the longest EffectWeight curve in that list. 
	The horizontal value of the curve represents time since the interaction start. The vertical value represents the EffectWeight of it's channel. 
	So if we had a EffectWeight curve for PositionWeight with 3 keyframes {(0, 0), (1, 1), (2, 0)} where a keyframe represents (time, value), then the positionWeight of an effector will reach 1 at 1 second from the interaction start and fall back to 0 at 2 secods from the interaction start.
	...
	<BR><b>PoserWeight</b> - EffectWeight of the hand/generic Poser.

	- <b>multipliers</b> - the EffectWeight curve multipliers are designed for reducing the amount of work with AnimationCurves. If you needed the rotationWeight curve to be the same as the positionWeight curve, you could use a multipier instead of duplicating the curve.
...
<b>Component variables:</b>
	...
	- <b>multipliers</b> - the EffectWeight curve multipliers enable you to override EffectWeight curve values for different effectors.
	...
	- <b>twistWeight</b> - the EffectWeight of twisting the interaction target towards the effector bone in the start of the interaction.
	- <b>swingWeight</b> - the EffectWeight of swinging the interaction target towards the effector bone in the start of the interaction. This will make the direction from the pivot to the InteractionTarget match the direction from the pivot to the effector bone.
...
- create an InteractionObject to interact with, add a EffectWeight curve, for example {(0, 0), (1, 1), (2, 0)}, for PositionWeight,
...
<b>Solver variables:</b>
		...
		- <b>footRotationWeight</b> - the EffectWeight off offsetting the rotation of the feet (range: 0 - 1).
		...
		- <b>lowerPelvisWeight</b> - the EffectWeight of moving the pelvis down to the lowest foot. This moves the pelvis down when the character walks down the stairs and needs to reach a lower step.
		- <b>liftPelvisWeight</b> - the EffectWeight of moving the pelvis up to the highest foot. This moves the pelvis up when the character steps on a higher obstacle. Use this when the character is crouching.
		...
		- <b>quality</b> - the quality mainly determines the EffectWeight of ray/sphere/acpsule casting. The Fastest will only make a single raycast for each foot, plus a raycast for the root. Simple will make 3 raycasts for each foot, plus a raycast for the root.
...
<b>Component variables:</b>
		- <b>EffectWeight</b> - the master EffectWeight. The effect of the Grounder can be smoothly faded out for better performance when not needed.
...
<b>Component variables:</b>
		...
		- <b>rootRotationWeight</b> - the EffectWeight of root rotation (range: 0 - 1).
...
<b>Component variables:</b>
		...
		- <b>rootRotationWeight</b> - the EffectWeight of root rotation. If the quadruped is standing on an edge and leaning down with the forelegs entering the ground, you need to increase this value and also the root rotation limits (range: 0 - 1).
		- <b>rootRotationSpeed</b> - the speed of root rotation interpolation (range: 0 - inf).
		...
		- <b>maintainHeadRotationWeight</b> - the EffectWeight of maintaining the head's rotation as animated (range: 0 - 1).
