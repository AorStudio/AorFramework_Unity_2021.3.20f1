...
namespace NodeGraph.Editor
{
    ...
    public class AssetPrefabProcessorGUIController : NodeGUIController
    {
        ...
        public override void DrawNodeInspector(float inspectorWidth)
        {
            ...
            string oguid = (string)m_nodeGUI.data.GetPublicField("CustomScriptGUID");
            if (!string.IsNullOrEmpty(oguid))
            {
                ...
                if (!string.IsNullOrEmpty(customPath))
                {
                    ...
                    if (custom != null)
                    {
                        ...
                        if (n == null)
                        {
                            m_nodeGUI.data.SetPublicField("CustomScriptGUID", null);
                            m_nodeGUI.SetDirty();
                        }
                        else if (n != custom)
                        {
                            ...
                            {
                                if((bool)m_nodeGUI.controller.InvokeNonPublicMethod("_getCustomScript", new object[] { guid })) { 
                                    m_nodeGUI.data.SetPublicField("CustomScriptGUID", guid);
                                    m_nodeGUI.SetDirty();
                                }
                            }
                        ...
                        //显示描述
                        string des = (string) m_nodeGUI.data.GetPublicField("CustomScriptDescribe");
                        ...
                        //显示自定义脚本字段
                        object cs = m_nodeGUI.controller.GetNonPublicField("_customScript");
                        if (cs != null)
                        {
                            //FieldInfo[] _customScriptFieldInfos
                            FieldInfo[] fieldInfos = (FieldInfo[])m_nodeGUI.controller.GetNonPublicField("_customScriptFieldInfos");
                    ...
                    {
                        m_nodeGUI.data.SetPublicField("CustomScriptGUID", null);
                        m_nodeGUI.data.SetPublicField("CustomScriptDescribe", null);
                        m_nodeGUI.SetDirty();
                    }
                }
                else
            ...
            //显示结果信息
            string[] scriptResultInfo = (string[]) m_nodeGUI.data.GetPublicField("CustomScriptResultInfo");
            if (scriptResultInfo != null)
            {

                GUILayout.Space(10);
                string rid = (string)m_nodeGUI.data.GetPublicField("ResultInfoDescribe");
