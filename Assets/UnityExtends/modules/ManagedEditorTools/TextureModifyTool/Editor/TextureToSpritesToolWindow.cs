...
namespace UnityEngine.Rendering.Universal.Editor.Utility
{
    ...
    public class TextureToSpritesToolWindow : UnityEditor.EditorWindow
    {
        ...
        [MenuItem("Window/FrameworkTools/Bitmaps/Sprites 图集数据导入工具")]
        ...
        private void _draw_toolTitle_UI()
        {
            ...
            GUILayout.Label("提示: 支持TexturePackerGUI(v3.0.+)DataFormat为 JSON(Array) / Unity3D 的Json描述文件");
            GUILayout.FlexibleSpace();
            GUILayout.EndHorizontal();
        ...
        private void _draw_importSprites_UI()
        {
            ...
            if (_targetTex && _targetJson)
            {
                ...
                {
                    ImportSpriteData(_targetTex, CreateAndCheckJSONFile(_targetJson), _keepOldPovitData);
                }
            }
            else
            {
        ...
        private static TPPackageData CreateAndCheckJSONFile(TextAsset textAsset)
        {
            ...
            {
                case ".txt":
                    return TPPackageData.ParseJSONHashFormatText(textAsset.text);
                default: // ".json";
                    return TPPackageData.ParseJSONArrayFormatText(textAsset.text);
            }
        }

        private static void ImportSpriteData(Texture2D tex, TPPackageData data, bool keepOldData)
        {

            string path = AssetDatabase.GetAssetPath(tex);
            TextureImporter textureImporter = AssetImporter.GetAtPath(path) as TextureImporter;
            if (!textureImporter || data == null || data.frames == null || data.frames.Length == 0)
                return;
            ...
            //float.Parse(metaSize["h"].ToString());
            float texH = (float)data.meta.size.h;
            ...
            for (int i = 0; i < data.frames.Length; i++) 
            {
                ...
                float y = texH - frameData.frame.y - h;

                Vector2 center = new Vector2(0.5f, 0.5f);
                bool trimmed = frameData.trimmed;
                if (trimmed)
                {
                    float orgW = frameData.sourceSize.w;//float.Parse(sourceSize["w"].ToString());
                    float orgH = frameData.sourceSize.h;//float.Parse(sourceSize["h"].ToString());
                    float spriteX = frameData.spriteSourceSize.x;//float.Parse(spriteSourceSize["x"].ToString());
                    float spriteY = frameData.spriteSourceSize.y;//float.Parse(spriteSourceSize["y"].ToString());
                    float spriteW = frameData.spriteSourceSize.w;// float.Parse(spriteSourceSize["w"].ToString());
                    float spriteH = frameData.spriteSourceSize.h; //float.Parse(spriteSourceSize["h"].ToString());
