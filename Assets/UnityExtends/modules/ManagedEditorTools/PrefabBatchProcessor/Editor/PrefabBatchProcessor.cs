...
namespace UnityEngine.Rendering.Universal.Utility.Editor
{
    ...
    public class PrefabBatchProcessor :UnityEditor.EditorWindow
    {
        ...
        [MenuItem("Window/FrameworkTools/Prefab/Prefab批量处理工具", false, 10100)]
        ...
        private void _draw_start_UI()
        {
            ...
            GUILayout.BeginHorizontal();
            {
                ...
                if(m_info.Length > 0)
                {
                    if (GUILayout.Button("[打印报告]", GUILayout.Height(36), GUILayout.Width(120)))
                    {
                        string[] logs = m_info.ToString().Replace("Prefab> ", "|").Split('|');
                        foreach(string log in logs) 
                        {
                            if (string.IsNullOrEmpty(log) || !log.Contains("Node>")) continue;
                            var slog = log.Replace("\r", "").Replace("\n", "");
                            Debug.Log(slog);
                        }
                    }
                    if (GUILayout.Button("[导出日志]", GUILayout.Height(36), GUILayout.Width(120)))
                    {
