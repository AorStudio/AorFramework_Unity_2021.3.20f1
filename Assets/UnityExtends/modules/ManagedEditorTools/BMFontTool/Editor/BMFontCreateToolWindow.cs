...
namespace UnityEngine.Rendering.Universal.Editor.Utility
{
    public class BMFontCreateToolWindow : UnityEditor.EditorWindow
    {
        ...
        [MenuItem("Window/FrameworkTools/BMFont/BMFont生成工具")]
        ...
        private void _draw_create_UI()
        {
            ...
            GUILayout.BeginHorizontal();
            {
                ...
                {
                    _fotTex = (Texture2D)EditorGUILayout.ObjectField(_fotTex, typeof(Texture2D), false);
                ...
                if (GUILayout.Button("SetFromSelection", GUILayout.Height(28)))
                {
                    foreach (UnityEngine.Object o in Selection.objects)
                    {
                        ...
                        {
                            EditorAssetInfo info = new EditorAssetInfo(AssetDatabase.GetAssetPath(o));
                            if(info.suffix == ".fnt" || info.suffix == ".txt" || info.suffix == ".json")
                            {
        ...
        private static void createBMFont(TextAsset import, Texture2D tex)
        {
            ...
            UnityEngine.CharacterInfo[] characterInfo = new UnityEngine.CharacterInfo[mbFont.glyphs.Count];
            for (int i = 0; i < mbFont.glyphs.Count; i++)
            {
                BMGlyph bmInfo = mbFont.glyphs[i];
                UnityEngine.CharacterInfo info = new UnityEngine.CharacterInfo();
                //设置ascii码  
